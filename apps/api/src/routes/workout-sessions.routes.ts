...
workoutSessionRoutes.patch(
  ...
  async (c) => {
    ...
    const startedAt = Date.now();
    c.header('x-request-id', requestId);
    console.info('[workout-sessions.update] request started', { requestId });

    try {
      const context = await resolveWorkoutContext(c.req.raw);
      if (!context) {
        console.info('[workout-sessions.update] context missing', { requestId });
        return c.json({ error: 'No workspace membership found', requestId }, 403);
      }
      ...
      const queryEndedAt = c.req.query('endedAt');
      const queryPerceivedIntensity = c.req.query('perceivedIntensity');
      const payload = {
        ...bodyRead.body,
        ...(c.req.query('sessionStatus') !== undefined
          ? { sessionStatus: c.req.query('sessionStatus') }
          : {}),
        ...(queryEndedAt !== undefined
          ? { endedAt: queryEndedAt === 'null' ? null : queryEndedAt }
          : {}),
        ...(c.req.query('notes') !== undefined
          ? { notes: c.req.query('notes') === 'null' ? null : c.req.query('notes') }
          : {}),
        ...(queryPerceivedIntensity !== undefined
          ? {
              perceivedIntensity:
                queryPerceivedIntensity === 'null'
                  ? null
                  : Number(queryPerceivedIntensity),
            }
          : {}),
      };

      const parsedPayload = updateWorkoutSessionSchema.safeParse(payload);
      if (!parsedPayload.success) {
        console.info('[workout-sessions.update] invalid payload', {
          requestId,
          issues: parsedPayload.error.issues,
          parseTimedOut: bodyRead.parseTimedOut,
          parseError: bodyRead.parseError,
        ...
        return c.json(
          {
            code: 'VALIDATION_ERROR',
            message: 'Invalid workout session update payload',
            details: parsedPayload.error.issues,
            requestId,
          },
          400,
      ...
      const data = parsedPayload.data;
      console.info('[workout-sessions.update] context resolved', {
        requestId,
        userId,
        workspaceId,
        sessionId,
        hasEndedAt: data.endedAt !== undefined,
        sessionStatus: data.sessionStatus ?? null,
        hasNotes: data.notes !== undefined,
        hasPerceivedIntensity: data.perceivedIntensity !== undefined,
        parseTimedOut: bodyRead.parseTimedOut,
        parseError: bodyRead.parseError,
      });
      ...
      if (!hasUpdates) {
        console.info('[workout-sessions.update] no fields to update', {
          requestId,
          elapsedMs: Date.now() - startedAt,
        });
        return c.json({ error: 'No fields to update', requestId }, 400);
      }
      ...
      if (!updatedSession) {
        console.info('[workout-sessions.update] response sent', {
          requestId,
          status: 404,
          elapsedMs: Date.now() - startedAt,
        });
        return c.json({ error: 'Workout session not found', requestId }, 404);
      }
      ...
      return c.json({ ...updatedSession, requestId });
    } catch (error) {
      console.error('[workout-sessions.update] failed', {
        requestId,
        elapsedMs: Date.now() - startedAt,
        error: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined,
      ...
      return c.json(
        {
          code: 'WORKOUT_SESSION_UPDATE_FAILED',
...
workoutSessionRoutes.post(
  ...
  async (c) => {
    ...
    const startedAt = Date.now();
    c.header('x-request-id', requestId);
    console.info('[workout-sets.create] request started', { requestId });

    try {
      const context = await resolveWorkoutContext(c.req.raw);
      if (!context) {
        console.info('[workout-sets.create] context missing', { requestId });
        return c.json({ error: 'No workspace membership found', requestId }, 403);
      }
      ...
      const payload = {
        ...bodyRead.body,
        ...(c.req.query('workoutSessionExerciseId') !== undefined
          ? { workoutSessionExerciseId: c.req.query('workoutSessionExerciseId') }
          : {}),
        ...(c.req.query('setNumber') !== undefined
          ? { setNumber: Number(c.req.query('setNumber')) }
          : {}),
        ...(c.req.query('weightValue') !== undefined
          ? { weightValue: Number(c.req.query('weightValue')) }
          : {}),
        ...(c.req.query('repsCount') !== undefined
          ? { repsCount: Number(c.req.query('repsCount')) }
          : {}),
        ...(c.req.query('durationSeconds') !== undefined
          ? { durationSeconds: Number(c.req.query('durationSeconds')) }
          : {}),
        ...(c.req.query('distanceMeters') !== undefined
          ? { distanceMeters: Number(c.req.query('distanceMeters')) }
          : {}),
        ...(c.req.query('rpe') !== undefined
          ? { rpe: Number(c.req.query('rpe')) }
          : {}),
        ...(c.req.query('rir') !== undefined
          ? { rir: Number(c.req.query('rir')) }
          : {}),
        ...(c.req.query('isWarmup') !== undefined
          ? { isWarmup: c.req.query('isWarmup') === 'true' }
          : {}),
        ...(c.req.query('isCompleted') !== undefined
          ? { isCompleted: c.req.query('isCompleted') === 'true' }
          : {}),
      };

      const parsedPayload = createWorkoutSetSchema.safeParse(payload);
      if (!parsedPayload.success) {
        console.info('[workout-sets.create] invalid payload', {
          requestId,
          issues: parsedPayload.error.issues,
          parseTimedOut: bodyRead.parseTimedOut,
          parseError: bodyRead.parseError,
      ...
      console.info('[workout-sets.create] context resolved', {
        requestId,
        userId,
        workspaceId,
        sessionId,
      ...
      if (!session) {
        console.info('[workout-sets.create] response sent', {
          requestId,
          status: 404,
      ...
      if (!sessionExercise) {
        console.info('[workout-sets.create] response sent', {
          requestId,
          status: 404,
          reason: 'session_exercise_not_found',
          elapsedMs: Date.now() - startedAt,
        });
        return c.json(
          {
      ...
      if (existingSet) {
        ...
        return c.json(
          { error: `Set #${setNumber} already exists for this exercise`, requestId },
          409,
        );
      }
      ...
      return c.json({ ...createdSet, requestId }, 201);
    } catch (error) {
      console.error('[workout-sets.create] failed', {
        requestId,
        elapsedMs: Date.now() - startedAt,
        error: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined,
      ...
      return c.json(
        {
          code: 'WORKOUT_SET_CREATE_FAILED',
